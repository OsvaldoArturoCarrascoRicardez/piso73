...
@section('content')
<?php 
date_default_timezone_set('America/Mexico_City'); //--added
        ...
        <div class="row">
            <div class="col-lg-2">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <h4 class="no-margins">{{$currency}} {{ format_numero( $today)}}</h4>
                    </div>
                </div>
            </div>
            <div class="col-lg-2">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <h4 class="no-margins">{{$currency}} {{ format_numero($yesterday) }}</h4>
			 ...
			 <div class="col-lg-2">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <h4 class="no-margins">{{$currency}} {{ format_numero($last_week) }}</h4>
			 ...
			 <div class="col-lg-2">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <h4 class="no-margins">{{$currency}} {{ format_numero($last_month) }}</h4>
			...
			<div class="col-lg-4">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <h4 class="no-margins">{{$currency}} {{ format_numero($total_earning) }}</h4>                        
                    </div>
                </div>
                @endif
            </div>
                    ...
                    @if (!empty($sales))
                        @forelse ($sales as $key => $sale)
                            <tr>
								...
								<td>{{-- $sale->created_at->format('Y-M-d') --}} 
                                    @if ( isset($sale->created_at) )    
                                    {{ formatFechaCortaM3($sale->created_at->timestamp) }}
                                    @endif
                                </td>
...
?>
            ...
            series: [
                    ...
                    data: [
                            ...
                            {
                                $array_day2[$value->dat] = $value->amount;
                            }

                             echo   '/*  data_ '.  json_encode($array_day2)   . '*/ ' ;   
                             echo   '/*  data_ '.  json_encode($day_name)   . '*/ ' ;   

                            /*foreach ($day_name as $key => $value) */
                            for ($i = 0; $i < 7; $i++){
                                $date2 = date('d', strtotime('- ' . $i . ' day'));
                                if (array_key_exists($date2, $array_day2))
                                {
                                    echo round($array_day2[$date2], 2) . ', ';
                                }
                                else
                                {
                                    echo '0.00, ';
