            ...
            <div class="form-group">
              ...
              <div class="col-sm-3">
				...
				<input type="date" class="form-control" id="porfecha" name="porfecha" placeholder="" 
                  @if($usrPermiso != null )
                    @if( !$usrPermiso->can('expence_date_update') )
                      readonly
                    @endif
                  @else 
                    readonly 
			...
			<label class="control-label col-sm-2" for="importe">Importe: </label> 
				<div class="col-sm-10">
				  <div class="input-group">
					...
					<!-- input id="price" name="price" type="number" class="form-control"   required    value="{{ format_numero( $gastos->expense_amount ) }}"  step="00000000.01"  {{-- step="0.01" --}} / -->
					<input id="importe" name="importe" type="text" class="form-control"  required  value="{{ $gastos->expense_amount }}"  step="0.01"  />	
...
input[type=number] {
  -moz-appearance: textfield;
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <input type="text" class="form-control" id="description" name="description"  
                	placeholder="Escriba una descripci�n breve. Ejem: Pan"  value="{{ $gastos->description }}"   required  />	
              </div>
            </div>
            {{--  Opcional a agregar...  --}}
            <!-- div class="form-group">
			...
			<div class="form-group"  id='filaCantidades'  style="<?php
				if ($gastos->unit_value == '0000'){
					echo 'display: none;';
				}
              ...
              <div class="col-sm-10">
				  ...
				  <label class="my-1 mr-2" for="lblCabtidad">Cantidad:</label>
					<div class="input-group col-sm-2">
						...
						value="{{ $gastos->quantity }}"  step="0.01" placeholder="Ejem: 1">
            ...
            <div class="form-group">
              <div class="col-sm-8 col-sm-offset-2">
                <a class="btn btn-white" href="{{ route('fondocaja.index') }}"> Regresar </a>
...
<script>
	...
	//  { { $gastos->created_at }}
	$('#porfecha').attr('value', '{{ date('Y-m-d', strtotime($gastos->created_at)) }}');
	// date('Y-m-d', strtotime($fecha1 . ' +1 day' ));
	...
	function _submit(e){
		...
		$.ajax( {
				...
				//__  url: '{ { route('fondocaja.update', $gastos->id)  } }',				gastos.update
				url: '{{ route('fondocaja.update') }}',				
