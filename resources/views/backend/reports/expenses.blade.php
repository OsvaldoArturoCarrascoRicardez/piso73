...
<div class="wrapper wrapper-content animated fadeInRight">
                    ...
                    @if (!empty($expenses))
                        @forelse ($expenses as $key => $expense)
                            <tr>
							   ...
							   <td>{{ formatFecha_d_Mes_a(strtotime($expense->created_at)) }}</td>
                                
							   <!-- td>{{ $expense->title}}</td -->  
							   <td>{{ $expense->description}}</td>
							   <td>{{$currency}} {{ format_numero($expense->expense_amount) }}</td>
                   
                           
                            </tr>
							<?php $total_amount += $expense->expense_amount; ?>
                        @empty
            ...
            <div class="panel panel-default">
                 <div class="ibox-title">
                        <h5>Mes disponible</h5>
                        
                    </div>
                 <div class="ibox-content">    
                    <form action="{{ route('reportes.gastos') }}" method="GET">
						...
						<div class="form-group" id="data_5">
								...
								<label class="font-noraml">Selecione</label>
                                <div class="input-daterange input-group" >
                                    
									<select name="mes" id="mes" class="input-sm form-control">
